...
import { FireflyAsset, FireflyInventoryResponse, FireflyAssetFilters } from './types';
...
export class FireflyClient {
   ...
   */
  async getAllAssets(filters?: FireflyAssetFilters, pageSize: number = 10000): Promise<FireflyAsset[]> {
    ...
    while (hasMore) {
      ...
      while (retries < 3) {
        try {
          ...
          const timeout = 1000 * retries;
          await new Promise(resolve => setTimeout(resolve, timeout)); // Exponential backoff
        }
      }
